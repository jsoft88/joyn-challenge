...
import com.joyn.challenge.stream.config.{CLIParams, ParamsBuilder}
import com.joyn.challenge.stream.factory.TransformationFactory
...
class TestChallengeTransformation extends FunSuite with BeforeAndAfterAll {
  val spark: SparkSession = SparkSession.builder().appName("TestTopPages").master("local[4]").getOrCreate()
  import spark.implicits._

  implicit val sqlCtx = spark.sqlContext
  val topics = Seq("pageviews", "users")
  ...
  test("Output is correct") {
    ...
    val pageViewsSchema = com.joyn.challenge.stream.jobs.kafka.PageViewsStream.SchemaByType.get(com.joyn.challenge.stream.jobs.kafka.PageViewsStream.PageViewsSchema)
    val usersSchema = com.joyn.challenge.stream.jobs.kafka.PageViewsStream.SchemaByType.get(com.joyn.challenge.stream.jobs.kafka.PageViewsStream.UsersSchema)
    ...
    val paramsForTransformation = new ParamsBuilder()
      ...
      .withSchemaTypePerTopic(topics(0), "pageviews")
...
//        )
  ...
  override def beforeAll() {
    ...
    val usersFile: InputStream = getClass.getResourceAsStream("/kafka/users.json")
    val pageviewsFile: InputStream = getClass().getResourceAsStream("/kafka/pageviews.json")
